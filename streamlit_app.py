...
def _inner_defaultdict():
    return defaultdict(float)  # why: pickle-friendly vs lambda


def _empty_agg():
    return defaultdict(_inner_defaultdict)
...
def _fast_excel_datetime(s: pd.Series) -> pd.Series:
    """Why: jauh lebih cepat untuk serial Excel; fallback ke parser umum."""
...
def _process_xlsb(data_or_buf: Union[bytes, BinaryIO], year: int, month: int, agg) -> None:
    """Optimized .xlsb streaming: direct index, big batch, fast datetime."""
    try:
        try:
...
# =========================== Loader Payment (parallel + cache-safe) ===========================
...
def _freeze_agg(agg):
    """Why: hasil cache harus pickleable; ubah nested defaultdict ke dict biasa."""
...
def main() -> None:
    ...
    finnet_espay_files = st.sidebar.file_uploader(  # placeholder jika nanti dipakai

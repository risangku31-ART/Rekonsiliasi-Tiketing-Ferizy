...
# =========================== Konfigurasi: Tabel ESPAY ===========================
ESPAY_DATE = "Settlement Date"   # tanggal di file ESPAY
ESPAY_PROD = "Product Name"      # tentukan VA vs E-Money
ESPAY_VA_NAME = "VA Name"        # mapping pelabuhan dari VA Name
...
# >>> Perbaiki pemetaan: pola lebih longgar (berbagai variasi di VA Name)
PORT_PATTERNS = {
...
def _port_from_va_name(val: str) -> str:
    # longgar: kenali "asdp merak" maupun "merak" saja, dst.
...
# =========================== Render helper ===========================
def _render_table(df_port: pd.DataFrame, highlight: bool = False) -> pd.DataFrame:
...
# =========================== Streamlit UI ===========================
def main() -> None:
    st.set_page_config(page_title="Rekonsiliasi & Settlement ESPAY (Cepat)", layout="wide")
    st.title("Rekonsiliasi Payment Report & Settlement Dana ESPAY (Mode Cepat)")
    ...
    max_workers = st.sidebar.slider("Paralel file (workers)", min_value=1, max_value=8, value=4)
    ...
    # ================== TABEL 2: Settlement Dana ESPAY (tampilan per Pelabuhan) ==================
    st.divider()
    ...
    # Tabs per Pelabuhan (seperti Tabel Utama)
    ports2 = list(result_espay["Pelabuhan"].dropna().unique()); ports2.sort()
    tabs2 = st.tabs(ports2 if ports2 else ["(Tidak ada Pelabuhan)"])
    for tab, port in zip(tabs2, ports2):
        with tab:
            st.markdown(f"**Pelabuhan: {port}**")
            _render_table(result_espay[result_espay["Pelabuhan"] == port], highlight=False)

    # Unduh gabungan ESPAY
    st.divider()
    st.subheader("Unduh Settlement Dana ESPAY (Gabungan)")
    export_df2 = result_espay.copy()
    export_df2["Tanggal"] = pd.to_datetime(export_df2["Tanggal"]).dt.strftime("%d/%m/%Y")
    num_cols2 = export_df2.select_dtypes(include="number").columns
    export_df2[num_cols2] = export_df2[num_cols2].round(0).astype("Int64")
    csv_bytes2 = export_df2.to_csv(index=False).encode("utf-8-sig")
    st.download_button("Unduh CSV (ESPAY)", data=csv_bytes2, file_name=f"settlement_espay_{year}_{month:02d}.csv", mime="text/csv")
    excel_bytes2, engine_used2, err_msg2 = _to_excel_bytes(export_df2, sheet_name="Settlement ESPAY")
    if excel_bytes2:

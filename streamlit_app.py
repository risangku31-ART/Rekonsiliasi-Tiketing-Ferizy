...
# =========================== Konstanta ===========================
COL_H = "TIPE PEMBAYARAN"; COL_B = "TANGGAL PEMBAYARAN"; COL_AA = "REF NO"
COL_K = "TOTAL TARIF TANPA BIAYA ADMIN (Rp.)"; COL_X = "SOF ID"; COL_ASAL = "ASAL"
...
DEFAULT_CSV_CHUNK_ROWS = 200_000; XLSX_BATCH_ROWS = 50_000; NONBCA_CREDIT_COL_INDEX = 9
    ...
    if highlight:
        for col in ["Selisih Tiket Detail vs Settlement Report","Selisih Dana Masuk vs Settlement Report","Selisih"]:
            if col in df_display.columns:
                styler = styler.apply(lambda s: ["background-color:#fdecea; color:#b71c1c; font-weight:600;"
                                                 if (pd.notna(v) and float(v) != 0) else "" for v in s], subset=[col])
    return styler
...
def _norm_colname(name: str) -> str:
    s = str(name); return "".join(ch.lower() for ch in s if ch.isalnum())
...
def _parse_amount_credit_series(s: pd.Series) -> pd.Series:
    x = s.astype(str)
    neg = (x.str.contains(r"\(", regex=True, na=False)
           | x.str.contains(r"\bDR\b", flags=re.I, regex=True, na=False)
           | x.str.contains("\u2212", regex=False, na=False)
           | x.str.strip().str.startswith("-"))
    x = x.str.replace(r"[()]", "", regex=True).str.replace("\u2212","-",regex=False)
    x = x.str.replace(r"\b(CR|DR)\b","",flags=re.I,regex=True).str.replace(r"[^0-9,.\-]","",regex=True)

    def _to_float(val: str) -> float:
        if not val: return 0.0
        if "," in val and "." in val:
            val = val.replace(".", "").replace(",", ".") if val.rfind(",")>val.rfind(".") else val.replace(",", "")
        else:
            if "," in val:
                a,b = val.rsplit(",",1); val = a.replace(",","")+("." + b if len(b) in (2,3) else "")
            elif "." in val:
                a,b = val.rsplit(".",1); val = a.replace(".","")+("." + b if len(b) in (2,3) else "")
        try: return float(val)
        except Exception: return 0.0

    vals = x.apply(_to_float); vals = vals.where(~neg, -vals.abs())
...
def _mask_remark_contains(remark: pd.Series, keywords: List[str]) -> pd.Series:
    ...
    norm = remark.astype(str).str.upper(); mask = pd.Series(False, index=remark.index)
    for k in keywords: mask = mask | norm.str.contains(str(k).upper(), na=False)
    return mask
...
def _to_date_minus1(v) -> pd.Series: return pd.to_datetime(v, errors="coerce", dayfirst=True) - pd.Timedelta(days=1)
...
def _port_from_bca_filename(fname: str) -> str:
    ...
    if "KETAPANG" in up: return "ASDP Bakauheni"  # by rule
    if "GILIMANUK" in up: return "ASDP Gilimanuk"
    return "ASDP Lainnya"

def _period_label(y: int, m: int) -> str:
    return pd.Timestamp(y, m, 1).strftime("%b-%y")  # e.g., Oct-25
...
def _update_agg_series(agg, ser: pd.Series, colname: str) -> None:
    if ser.empty: return
    for (dt, asal), val in ser.items(): agg[(dt, asal)][colname] += float(val)
...
def _apply_rules_and_update(df_chunk: pd.DataFrame, agg) -> None:
    ...
    amt = pd.to_numeric(df_chunk[COL_K], errors="coerce").fillna(0); tgl = df_chunk["Tanggal"]

    def sum_by_key(mask: pd.Series) -> pd.Series:
        if mask.any(): return amt[mask].groupby([tgl[mask], ASAL[mask]], dropna=False).sum(min_count=1)
        return pd.Series(index=pd.MultiIndex.from_arrays([[],[]], names=["Tanggal","Pelabuhan"]), dtype="float64")
    ...
    for name, m in rules.items(): _update_agg_series(agg, sum_by_key(m), name)
    ...
    is_not_spay = ~X.str.contains("spay", na=False); is_bca = X.str.contains("bca", na=False)
...
def _iter_csv_chunks(file_like: Union[io.BytesIO, io.BufferedReader], usecols: List[str], year: int, month: int,
    ...
    if _HAS_PYARROW:
        try:
            ...
            t = pd.to_datetime(df[COL_B], errors="coerce"); mask = (t.dt.year==year) & (t.dt.month==month)
            if mask.any(): sub = df.loc[mask].copy(); sub["Tanggal"] = t.loc[mask].dt.date; yield sub
            return
        except Exception: file_like.seek(0)
    for chunk in pd.read_csv(file_like, usecols=usecols, chunksize=chunksize, sep=delim, encoding="utf-8-sig",
                             dtype={COL_H:"string",COL_AA:"string",COL_X:"string",COL_ASAL:"string"},
                             on_bad_lines="skip", engine="python"):
        t = pd.to_datetime(chunk[COL_B], errors="coerce"); mask = (t.dt.year==year) & (t.dt.month==month)
        if mask.any(): sub = chunk.loc[mask].copy(); sub["Tanggal"] = t.loc[mask].dt.date; yield sub

# =========================== Payment loaders ===========================
def _process_csv_fast(data: bytes, year: int, month: int, agg, chunk_rows: int) -> None:
    for sub in _iter_csv_chunks(io.BytesIO(data), REQUIRED_COLS, year, month, chunksize=chunk_rows):
...
def _flush_xlsx_batch(buf: List[List], year: int, month: int, agg) -> None:
    ...
    t = pd.to_datetime(df[COL_B], errors="coerce"); mask = (t.dt.year==year) & (t.dt.month==month)
    if mask.any(): sub = df.loc[mask].copy(); sub["Tanggal"] = t.loc[mask].dt.date; _apply_rules_and_update(sub, agg)
...
def _process_xlsx_streaming(data: bytes, year: int, month: int, agg) -> None:
    try:
        ...
        if not all(c in idx for c in REQUIRED_COLS): wb.close(); return
        buf = []
        for r in rows:
            try:
                buf.append([r[idx[COL_H]], r[idx[COL_B]], r[idx[COL_AA]], r[idx[COL_K]], r[idx[COL_X]], r[idx[COL_ASAL]]])
            except Exception: continue
            if len(buf) >= XLSX_BATCH_ROWS: _flush_xlsx_batch(buf, year, month, agg); buf.clear()
...
def _process_payment_file(name: str, data: bytes, year: int, month: int, chunk_rows: int) -> dict:
    agg = _empty_agg(); low = str(name).lower()
    try:
        if low.endswith(".zip"):
            with zipfile.ZipFile(io.BytesIO(data)) as zf:
                for info in zf.infolist():
                    ...
                    elif fn.endswith(".csv"): _process_csv_fast(content, year, month, agg, chunk_rows)
...
def _merge_plain_aggs(target: dict, source: dict) -> None:
    for key, bucket in source.items():
        if key not in target: target[key] = dict(bucket)
            ...
            for k2, v2 in bucket.items(): target[key][k2] = float(target[key].get(k2,0.0)) + float(v2)
...
def load_and_aggregate_fast(files, year: int, month: int, max_workers: int, csv_chunk_rows: int) -> dict:
    ...
    def submit_one(executor, f):
        ...
        name = getattr(f,"name","file"); return executor.submit(_process_payment_file, name, data, year, month, csv_chunk_rows)
...
def _read_settlement_single_table(content: bytes, filename: str) -> Optional[pd.DataFrame]:
    ...
    try:
        if low.endswith(".csv"):
            buf = io.BytesIO(content); head = buf.read(2048); buf.seek(0); delim = _sniff_delimiter(head)
            if _HAS_PYARROW: return pd.read_csv(buf, sep=delim, engine="pyarrow")
            try: return pd.read_csv(buf, sep=delim, encoding="utf-8-sig", engine="python", on_bad_lines="skip")
            except UnicodeDecodeError: buf.seek(0); return pd.read_csv(buf, sep=delim, encoding="latin1", engine="python", on_bad_lines="skip")
        if low.endswith(".xlsx"): return pd.read_excel(io.BytesIO(content), engine="openpyxl")
    ...
    for f in files:
        ...
        try:
            ...
            if df_part is not None and not df_part.empty:
                ...
                rename_map = {}; reqs = ["Product Name","Settlement Amount","Settlement Date","VA NAME"]
                for req in reqs:
                    key = req.lower()
                    if key in lower_to_real: rename_map[lower_to_real[key]] = req
                df_part.rename(columns=rename_map, inplace=True)
                if all(c in df_part.columns for c in reqs):
                    all_dfs.append(df_part[reqs].copy())
        except Exception: continue
...
def _build_espay_settlement_table(df_settlement: pd.DataFrame, year: int, month: int) -> pd.DataFrame:
    ...
    df = df_settlement.copy(); t = pd.to_datetime(df["Settlement Date"], errors="coerce"); df["Tanggal"] = t.dt.date
    df = df.loc[(t.dt.year==year) & (t.dt.month==month)].copy(); if df.empty: return pd.DataFrame()
    ...
    def map_port(name: str) -> Optional[str]:
        ...
        return None
    df["Pelabuhan"] = va_name.apply(map_port); df = df[df["Pelabuhan"].notna()].copy()
    amt = pd.to_numeric(df["Settlement Amount"].astype(str).str.replace(r"[^\d\-]","",regex=True), errors="coerce").fillna(0.0)/100.0
    pn = df["Product Name"].fillna("").astype(str).str.lower()
    is_va = pn.str.contains("va", na=False); is_bca = pn.str.contains("bca", na=False) | pn.str.contains("blu", na=False)
    df["VIRTUAL ACCOUNT"] = amt.where(is_va, 0.0); df["E-MONEY"] = amt.where(~is_va, 0.0)
    df["BCA"] = amt.where(is_bca, 0.0); df["NON BCA"] = amt.where(~is_bca, 0.0)
    ...
    uniq = grouped["Pelabuhan"].dropna().unique(); 
    if len(uniq)==0: return pd.DataFrame()
    days = monthrange(year, month)[1]; all_dates = [date(year, month, d) for d in range(1, days+1)]
    out = pd.DataFrame(index=pd.MultiIndex.from_product([all_dates, uniq], names=["Tanggal","Pelabuhan"])).reset_index()
    out = out.merge(grouped, on=["Tanggal","Pelabuhan"], how="left").fillna(0.0)
...
def _load_settlement_finnet(files) -> pd.DataFrame:
            ...
            if name.endswith(".zip"):
                with zipfile.ZipFile(io.BytesIO(data)) as zf:
                    for info in zf.infolist():
                        ...
                        try: part = _read_finnet_single_csv(zf.read(info))
    ...
    norm = {c:_norm_colname(c) for c in df.columns}; rename_map = {}
    for req in ["Payment Method","Merchant Amount","Payment Date Time","Merchant Name"]:
        req_norm = _norm_colname(req)
        for real, nm in norm.items():
            if nm == req_norm or nm.startswith(req_norm) or req_norm.startswith(nm): rename_map[real]=req; break
...
def _build_finnet_settlement_table(df_finnet: pd.DataFrame, year: int, month: int) -> pd.DataFrame:
    ...
    df = df_finnet.copy(); need = ["Payment Date Time","Merchant Amount","Merchant Name","Payment Method"]
    if any(c not in df.columns for c in need): return pd.DataFrame()
    raw_dt = df["Payment Date Time"].astype(str).str.strip(); t = pd.to_datetime(raw_dt.str.slice(0,10), errors="coerce")
    df["Tanggal"] = t.dt.date; df = df.loc[(t.dt.year==year) & (t.dt.month==month)].copy(); 
    ...
    def map_port(name: str) -> str:
        ...
        if "MERAK" in name:     return "ASDP Merak"
        return "ASDP Lainnya"
    df["Pelabuhan"] = mn.apply(map_port)
    ...
    is_va = pm.str.contains("va", na=False); is_bca = pm.str.contains("bca", na=False) | pm.str.contains("blu", na=False)
    is_non_bca = ~(pm.str.contains("bca", na=False) | pm.str.contains("blu", na=False))
    df["VIRTUAL ACCOUNT"] = amt.where(is_va, 0.0); df["E-MONEY"] = amt.where(~is_va, 0.0)
    df["BCA"] = amt.where(is_bca, 0.0); df["NON BCA"] = amt.where(is_non_bca, 0.0)
    grouped = df.groupby(["Tanggal","Pelabuhan"], dropna=False)[["VIRTUAL ACCOUNT","E-MONEY","BCA","NON BCA"]].sum().reset_index()
    for c in ["VIRTUAL ACCOUNT","E-MONEY","BCA","NON BCA"]: grouped[c]=grouped[c].fillna(0.0)
    uniq = grouped["Pelabuhan"].dropna().unique(); 
    if len(uniq)==0: return pd.DataFrame()
...
def _read_any_table_with_header(content: bytes, filename: str, header_row: int) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        buf = io.BytesIO(content); head = buf.read(2048); buf.seek(0); delim = _sniff_delimiter(head)
        if _HAS_PYARROW: return pd.read_csv(buf, skiprows=skiprows, header=0, engine="pyarrow", sep=delim)
        try: return pd.read_csv(buf, skiprows=skiprows, header=0, encoding="utf-8-sig", sep=delim, engine="python", on_bad_lines="skip")
        except UnicodeDecodeError: buf.seek(0); return pd.read_csv(buf, skiprows=skiprows, header=0, encoding="latin1", sep=delim, engine="python", on_bad_lines="skip")
    except Exception: return None
...
def _read_bca_table_row2(content: bytes) -> Optional[pd.DataFrame]:
    ...
    if df is None:
        try:
            df = pd.read_csv(io.BytesIO(content), header=0, engine="pyarrow") if _HAS_PYARROW else pd.read_csv(io.StringIO(content.decode("utf-8-sig","ignore")), header=0)
        except Exception: return None
...
def _load_rk_bca_generic(files, keys: List[str], port_from_name) -> Dict[Tuple[date, str], float]:
    ...
    def extract(df: pd.DataFrame) -> Optional[pd.DataFrame]:
        ...
        sub = sub[sub["Tanggal"].notna()]; sub = sub[_mask_remark_contains(sub["Keterangan"], keys)]
        return sub[["Tanggal","Amount"]] if not sub.empty else None
    def handle_one(content: bytes, fname: str):
        port = port_from_name(fname); df = _read_bca_table_row2(content)
        if df is None or df.empty: return
        part = extract(df); 
        if part is None or part.empty: return
        for dt_val, amt in part.groupby("Tanggal")["Amount"].sum().items(): totals[(dt_val, port)] += float(amt)
    for f in files:
        try: data = f.getvalue() if hasattr(f,"getvalue") else f.read()
        except Exception: continue
        fname = f.name
...
def _load_rk_nonbca_generic(files, header_row: int, keys: List[str]) -> Dict[Tuple[date, str], float]:
    ...
    totals = defaultdict(float)
    def handle_one(content: bytes, fname: str):
        port = _port_from_filename(fname); df = _read_any_table_with_header(content, fname, header_row)
        ...
        sub = sub[sub["Tanggal"].notna()]; sub = sub[_mask_remark_contains(sub["Remark"], keys)]
...
def _append_ketapang_gilimanuk_combined(df: pd.DataFrame, final_cols: list) -> pd.DataFrame:
    ...
    mask = df["Pelabuhan"].isin(["ASDP Ketapang","ASDP Gilimanuk"]); 
...
def _build_finnet_rekon_table(agg, df_finnet_settlement: Optional[pd.DataFrame], year: int, month: int,
    ...
    days = monthrange(year, month)[1]; all_dates = [date(year,month,d) for d in range(1,days+1)]
    base_df = pd.DataFrame(index=pd.MultiIndex.from_product([all_dates, unique_ports], names=["Tanggal","Pelabuhan"])).reset_index()
    ...
    for (dt, asal), bucket in agg.items():
        ...
        bca_val = float(bucket.get("FINNET_TIKET_BCA",0.0)); non_bca_val = float(bucket.get("FINNET_TIKET_NON_BCA",0.0))
    ...
    ticket_df = pd.DataFrame(rows).groupby(["Tanggal","Pelabuhan"],as_index=False)[["Tiket_BCA","Tiket_NON_BCA"]].sum() if rows else pd.DataFrame(columns=["Tanggal","Pelabuhan","Tiket_BCA","Tiket_NON_BCA"])
    ...
    else: settle_df = pd.DataFrame(columns=["Tanggal","Pelabuhan","BCA","NON BCA"])
    ...
    out["Tiket Detail - BCA"] = out["Tiket_BCA"]; out["Tiket Detail - Non BCA"] = out["Tiket_NON_BCA"]
    out["Settlement Report - BCA"] = out["BCA"]; out["Settlement Report - Non BCA"] = out["NON BCA"]

    bca_map = bca_inflow_by_dt_port or {}; nonbca_map = nonbca_inflow_by_dt_port or {}
    ...
    final_cols = ["Tanggal","Pelabuhan","Tiket Detail - BCA","Tiket Detail - Non BCA","Total Tiket Detail",
...
def _build_espay_rekon_table(agg, df_espay_settlement: Optional[pd.DataFrame], year: int, month: int,
                             ...
                             nonbca_inflow_by_dt_port_sgw: Optional[Dict[Tuple[date,str],float]]=None) -> pd.DataFrame:
    ports_from_payment = {_canonical_port_name(asal) for (_,asal) in agg.keys() if asal is not None}
    ports_from_settle = set()
    ...
    for (dt, asal), bucket in agg.items():
        ...
        dt_val = dt if isinstance(dt, date) else pd.to_datetime(dt, errors="coerce").date()
        if dt_val is None or dt_val.year!=year or dt_val.month!=month: continue
        bca_val = float(bucket.get("ESPAY_TIKET_BCA",0.0)); non_bca_val = float(bucket.get("ESPAY_TIKET_NON_BCA",0.0))
        if bca_val==0.0 and non_bca_val==0.0: continue
        rows.append({"Tanggal":dt_val,"Pelabuhan":asal_norm,"Tiket_BCA":bca_val,"Tiket_NON_BCA":non_bca_val})
    ticket_df = pd.DataFrame(rows).groupby(["Tanggal","Pelabuhan"],as_index=False)[["Tiket_BCA","Tiket_NON_BCA"]].sum() if rows else pd.DataFrame(columns=["Tanggal","Pelabuhan","Tiket_BCA","Tiket_NON_BCA"])
    ...
    bca_map = bca_inflow_by_dt_port_sgw or {}; nonbca_map = nonbca_inflow_by_dt_port_sgw or {}
    out["Dana Masuk - BCA"] = out.apply(lambda r: float(bca_map.get((r["Tanggal"], _canonical_port_name(r["Pelabuhan"])), 0.0)), axis=1)
    out["Dana Masuk - Non BCA"] = out.apply(lambda r: float(nonbca_map.get((r["Tanggal"], _canonical_port_name(r["Pelabuhan"])), 0.0)), axis=1)
...
def _count_tx_finnet(df_raw: pd.DataFrame, year: int, month: int) -> pd.DataFrame:
    ...
    if any(c not in df_raw.columns for c in need): return pd.DataFrame(columns=["Pelabuhan","Jumlah Transaksi"])
...
def _build_summary_table(df_rekon: pd.DataFrame, df_counts: pd.DataFrame, year: int, month: int) -> pd.DataFrame:
    if df_rekon is None or df_rekon.empty: return pd.DataFrame()
    sums = df_rekon.groupby("Pelabuhan", as_index=False)[["Total Settlement Report","Total Tiket Detail"]].sum()
    if df_counts is None or df_counts.empty:
        counts = pd.DataFrame({"Pelabuhan": sums["Pelabuhan"], "Jumlah Transaksi": 0})
    else:
        counts = df_counts.copy()
    out = sums.merge(counts, on="Pelabuhan", how="left").fillna(0)
    out["Jumlah Transaksi"] = out["Jumlah Transaksi"].astype("Int64")
    out["Periode"] = _period_label(year, month)
    out = out[["Periode","Pelabuhan","Jumlah Transaksi","Total Settlement Report","Total Tiket Detail"]]
    out.rename(columns={
        "Pelabuhan": "Cabang",
        "Total Settlement Report": "Nominal Transaksi (inc fee)",
        "Total Tiket Detail": "Nominal Transaksi (exc fee)",
    }, inplace=True)
    # subtotal row
    subtotal = {
        "Periode": out["Periode"].iloc[0] if not out.empty else _period_label(year, month),
        "Cabang": "Total",
    ...
    # MultiIndex header to mimic screenshot
...
def _prep_numeric_dates(df: pd.DataFrame) -> pd.DataFrame:
    ...
    num_cols = out.select_dtypes(include="number").columns; out[num_cols] = out[num_cols].fillna(0).round(0).astype("Int64")
    return out
...
def _to_excel_workbook_bytes(df_payment: pd.DataFrame, df_rekon_finnet: pd.DataFrame, df_rekon_espay: pd.DataFrame,
    ...
    for engine in ("xlsxwriter","openpyxl"):
        ...
        except Exception as e: return None, None, f"Gagal menulis Excel ({engine}): {e}"
...
def _render_df(df_show: pd.DataFrame, highlight: bool, max_rows_style: int = 1500) -> None:
    df_show = df_show.copy(); df_show["Tanggal"] = pd.to_datetime(df_show["Tanggal"]).dt.strftime("%d/%m/%Y")
    ...
    if len(df_show) <= max_rows_style and highlight:
        try: st.dataframe(_style_table(df_show, True), use_container_width=True, height=520); return
        except Exception: pass
...
def main() -> None:
    ...
    today = date.today(); years_options = list(range(today.year-5, today.year+6))
    ...
    if col_btn[0].button("▶️ Mulai Proses", type="primary"): st.session_state.run_started=True; st.session_state.results={}
    if col_btn[1].button("🔁 Proses Ulang"): st.session_state.run_started=True; st.session_state.results={}
    if col_btn[2].button("🧹 Bersihkan Hasil"): st.session_state.run_started=False; st.session_state.results={}
    ...
    if "payment" not in results:
        if not up_files: st.error("Payment Report belum diupload."); return
    ...
    if df_payment.empty: st.warning("Tidak ada data valid periode ini."); return
    st.subheader(f"1) Hasil Rekonsiliasi Payment • {month_names[month]} {year}")
    ports = sorted(df_payment["Pelabuhan"].dropna().unique())
    if ports:
        (st.tabs(ports) if len(ports)<=6 else [None])
        if len(ports)<=6:
            for tab, port in zip(st.tabs(ports), ports):
                with tab: st.markdown(f"**Pelabuhan: {port}**"); _render_df(df_payment[df_payment["Pelabuhan"]==port], highlight=False)
        else:
            chosen = st.selectbox("Pilih Pelabuhan", ports, key="pay_sel"); _render_df(df_payment[df_payment["Pelabuhan"]==chosen], highlight=False)
    progress.progress(25)

    # 2) ESPAY
    st.divider(); st.subheader("2) DETAIL SETTLEMENT ESPAY")
    if "espay" not in results:
        with st.spinner("2/5 • Memproses Settlement ESPAY…"):
            if settlement_files:
                raw = _load_settlement_espay(settlement_files); results["espay_raw"] = raw
    ...
    if df_espay.empty: st.info("Settlement ESPAY kosong / tidak diupload / periode tidak cocok.")
    else:
        ports_espay = sorted(df_espay["Pelabuhan"].dropna().unique())
        if len(ports_espay)<=6:
            for tab, port in zip(st.tabs(ports_espay), ports_espay):
                with tab: st.markdown(f"**Pelabuhan: {port}**"); _render_df(df_espay[df_espay["Pelabuhan"]==port], highlight=False)
        else:
            chosen = st.selectbox("Pilih Pelabuhan (ESPAY)", ports_espay, key="espay_sel"); _render_df(df_espay[df_espay["Pelabuhan"]==chosen], highlight=False)
    progress.progress(45)

    # 3) FINNET by Telkom
    st.divider(); st.subheader("3) DETAIL SETTLEMENT FINNET BY TELKOM")
    if "finnet_telkom" not in results:
        with st.spinner("3/5 • Memproses Settlement FINNET (Telkom)…"):
            if finnet_files:
                raw = _load_settlement_finnet(finnet_files); results["finnet_telkom_raw"] = raw
    ...
    if df_finnet.empty: st.info("Settlement FINNET (Telkom) kosong / tidak diupload / periode tidak cocok.")
    else:
        ports_finnet = sorted(df_finnet["Pelabuhan"].dropna().unique())
        if len(ports_finnet)<=6:
            for tab, port in zip(st.tabs(ports_finnet), ports_finnet):
                with tab: st.markdown(f"**Pelabuhan: {port}**"); _render_df(df_finnet[df_finnet["Pelabuhan"]==port], highlight=False)
        else:
            chosen = st.selectbox("Pilih Pelabuhan (Finnet Telkom)", ports_finnet, key="finnet_telkom_sel"); _render_df(df_finnet[df_finnet["Pelabuhan"]==chosen], highlight=False)
    progress.progress(65)

    # 4) FINNET (ESPAY)
    st.divider(); st.subheader("4) DETAIL SETTLEMENT FINNET (ESPAY)")
    if "finnet_espay" not in results:
        with st.spinner("4/5 • Memproses Settlement FINNET (ESPAY)…"):
            if finnet_espay_files:
                raw = _load_settlement_finnet(finnet_espay_files); results["finnet_espay_raw"] = raw
    ...
    if df_finnet_espay.empty: st.info("Settlement FINNET (ESPAY) kosong / tidak diupload / periode tidak cocok.")
    else:
        ports_fe = sorted(df_finnet_espay["Pelabuhan"].dropna().unique())
        if len(ports_fe)<=6:
            for tab, port in zip(st.tabs(ports_fe), ports_fe):
                with tab: st.markdown(f"**Pelabuhan: {port}**"); _render_df(df_finnet_espay[df_finnet_espay["Pelabuhan"]==port], highlight=False)
        else:
            chosen = st.selectbox("Pilih Pelabuhan (Finnet Espay)", ports_fe, key="finnet_espay_sel"); _render_df(df_finnet_espay[df_finnet_espay["Pelabuhan"]==chosen], highlight=False)
    ...
    df_rekon_finnet = _build_finnet_rekon_table(agg, results.get("finnet_telkom", pd.DataFrame()),
                                                year, month, bca_finif, nonbca_finif)
    if df_rekon_finnet.empty: st.info("Tabel Rekonsiliasi Finnet belum dapat dibentuk.")
    else:
        ports_rekon = sorted(df_rekon_finnet["Pelabuhan"].dropna().unique())
        if len(ports_rekon)<=6:
            for tab, label in zip(st.tabs(ports_rekon), ports_rekon):
                with tab: st.markdown(f"**Pelabuhan: {label}**"); _render_df(df_rekon_finnet[df_rekon_finnet["Pelabuhan"]==label], highlight=True)
        else:
            chosen = st.selectbox("Pilih Pelabuhan (Rekon Finnet)", ports_rekon, key="rekon_finnet_sel"); _render_df(df_rekon_finnet[df_rekon_finnet["Pelabuhan"]==chosen], highlight=True)
    ...
    df_rekon_espay = _build_espay_rekon_table(agg, results.get("espay", pd.DataFrame()),
                                              year, month, bca_sgw, nonbca_sgw)
    if df_rekon_espay.empty: st.info("Tabel Rekonsiliasi ESPAY belum dapat dibentuk.")
    else:
        ports_rekon_e = sorted(df_rekon_espay["Pelabuhan"].dropna().unique())
        if len(ports_rekon_e)<=6:
            for tab, label in zip(st.tabs(ports_rekon_e), ports_rekon_e):
                with tab: st.markdown(f"**Pelabuhan: {label}**"); _render_df(df_rekon_espay[df_rekon_espay["Pelabuhan"]==label], highlight=True)
        else:
            chosen = st.selectbox("Pilih Pelabuhan (Rekon ESPAY)", ports_rekon_e, key="rekon_espay_sel"); _render_df(df_rekon_espay[df_rekon_espay["Pelabuhan"]==chosen], highlight=True)

    # === NEW: SUMMARY TABLES (look like screenshot) ===
    ...
    with st.expander("Summary • FINNET", expanded=True):
        ...
        sum_finnet = _build_summary_table(df_rekon_finnet, finnet_counts, year, month)
    ...
    with st.expander("Summary • ESPAY", expanded=True):
        ...
        sum_espay = _build_summary_table(df_rekon_espay, espay_counts, year, month)
        _render_summary(sum_espay)

    progress.progress(100)

    # Unduh Excel (multi-sheet: Summary + Payment + Rekon)
    ...
    excel_bytes, engine_used, err_msg = _to_excel_workbook_bytes(
        results["payment"], df_rekon_finnet, df_rekon_espay, sum_finnet, sum_espay, year, month
    )
    if excel_bytes:
        st.download_button(f"Unduh Excel (.xlsx) • engine: {engine_used}",
                           data=excel_bytes,

...
# (potongan lengkap – ganti file Anda dengan ini)
...
def _detect_amount_col(cols: List[str]) -> Optional[str]:
    ...
    return None  # fallback hitung transaksi
...
def _apply_and_update_espay(df_chunk: pd.DataFrame, agg) -> None:
    ...
    if amt_col:
        ...
        amt = pd.Series(1.0, index=df_chunk.index)  # hitung transaksi
...
def _process_csv_espay(data: bytes, year: int, month: int, agg) -> Tuple[Optional[str], int, int]:
    """
    Return: (amount_col_used or None, total_rows_read, filtered_rows_used)
    - CSV dibaca per chunk TANPA usecols -> header di-trim agar toleran.
    """
    itr = pd.read_csv(io.BytesIO(data), chunksize=CSV_CHUNK_ROWS, dtype="unicode")
    amount_used = None; total_rows = 0; used_rows = 0
    for chunk in itr:
        total_rows += len(chunk)
        chunk.columns = [str(c).strip() for c in chunk.columns]  # penting: trim header
        # parse tanggal dd/mm/yyyy umum di ID
...
def _process_xlsx_espay(data: bytes, year: int, month: int, agg) -> Tuple[Optional[str], int, int]:
    """Streaming openpyxl; fallback ke pandas.read_excel. Return (amount_col_used, rows_read, rows_used)."""
    ...
    try:
        ...
        for r in rows:
            ...
            try:
                ...
                continue
            if len(buf) >= XLSX_BATCH_ROWS:
                used = _flush_xlsx_batch_espay(buf, year, month, agg, amt_col_name)
                rows_used += used
...
def _flush_xlsx_batch_espay(buf: List[List], year: int, month: int, agg, amt_col_name: Optional[str]) -> int:
                ...
                with zipfile.ZipFile(io.BytesIO(data)) as zf:
                    for m in zf.infolist():
                        ...
                        if flow.endswith((".xlsx", ".xls")):
                            c, r_all, r_used = _process_xlsx_espay(content, year, month, agg)
                        else:
                            c, r_all, r_used = _process_csv_espay(content, year, month, agg)
                        info.append(f"{name}::{fname} → {'sum ' + c if c else 'count trx'} (rows: {r_used}/{r_all} terpakai)")
            elif low.endswith((".xlsx", ".xls")):
                c, r_all, r_used = _process_xlsx_espay(data, year, month, agg)
                info.append(f"{name} → {'sum ' + c if c else 'count trx'} (rows: {r_used}/{r_all} terpakai)")
            elif low.endswith(".csv"):
                c, r_all, r_used = _process_csv_espay(data, year, month, agg)
                info.append(f"{name} → {'sum ' + c if c else 'count trx'} (rows: {r_used}/{r_all} terpakai)")
        except Exception as e:

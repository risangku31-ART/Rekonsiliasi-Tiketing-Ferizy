...
# RK Non BCA: baris 13..1000, kolom Credit = J (1-based 10 → 0-based 9)
NONBCA_START_ROW = 13
NONBCA_END_ROW = 1000
NONBCA_CREDIT_COL_INDEX = 9  # J
...
def _format_rp_signed(v: float) -> str:
    # why: tampilkan +/− dengan pemisah ribuan titik
    try:
        f = float(v)
    except Exception:
        return ""
    sign = "+" if f >= 0 else "-"
    body = f"{abs(f):,.0f}".replace(",", ".")
    return f"{sign}{body}"
...
def _process_csv_fast(data: bytes, year: int, month: int, agg) -> None:
    itr = pd.read_csv(io.BytesIO(data), usecols=REQUIRED_COLS, chunksize=CSV_CHUNK_ROWS,
...
def _build_espay_settlement_table(df_settlement: pd.DataFrame, year: int, month: int) -> pd.DataFrame:
    ...
    mask = (t.dt.year == year) & (t.dt.month == month); df = df.loc[mask].copy()
    ...
    out = pd.DataFrame(index=full_idx).reset_index().merge(grouped, on=["Tanggal","Pelabuhan"], how="left").fillna(0.0)
    ...
    existing = [c for c in desired_order if c in out.columns]; others = [c for c in out.columns if c not in existing]
    return out[existing + others].sort_values(["Pelabuhan","Tanggal"]).reset_index(drop=True)
...
def _build_finnet_settlement_table(df_finnet: pd.DataFrame, year: int, month: int) -> pd.DataFrame:
    ...
    pm_lower = df["Payment Method"].fillna("").astype(str).str.lower()
    ...
    all_dates = [date(year, month, d) for d in range(1, days_in_month + 1)]
    full_idx = pd.MultiIndex.from_product([all_dates, unique_ports], names=["Tanggal","Pelabuhan"])
    out = (pd.DataFrame(index=full_idx).reset_index()
           .merge(grouped, on=["Tanggal","Pelabuhan"], how="left").fillna(0.0))
    out["TOTAL VA + E-MONEY"] = out["VIRTUAL ACCOUNT"] + out["E-MONEY"]
    out["TOTAL BCA + NON BCA"] = out["BCA"] + out["NON BCA"]
    desired_order = ["Tanggal","Pelabuhan","VIRTUAL ACCOUNT","E-MONEY","TOTAL VA + E-MONEY","BCA","NON BCA","TOTAL BCA + NON BCA"]
    existing = [c for c in desired_order if c in out.columns]; others = [c for c in out.columns if c not in existing]
...
# =========================== RK Non BCA: baca posisional (baris 13–1000, Credit@J) ===========================
def _read_rk_nonbca_positional(content: bytes) -> Optional[pd.DataFrame]:
    # A=Date(0), Remark=C..F (2..5), Credit=J(9), baris 13..1000
    ...
    if df.shape[1] <= NONBCA_CREDIT_COL_INDEX: return None
    ...
    amount = pd.to_numeric(credit_raw.str.replace(r"\s+", "", regex=True)
...
def _load_rek_koran_nonbca_by_port(files: List["st.runtime.uploaded_file_manager.UploadedFile"], remark_codes: List[str]) -> Dict[Tuple[date, str], float]:
    ...
    def add_from_content(content: bytes, filename_hint: str) -> None:
        ...
        g = df.assign(Date=df["Tanggal"].dt.date).groupby("Date")["Amount"].sum()
...
# -------- Preview RK Non BCA (BARIS, TERFORMAT) --------
def _preview_rk_nonbca_rows_formatted(files: List["st.runtime.uploaded_file_manager.UploadedFile"],
                                      remark_codes: List[str],
                                      max_rows: int = 200) -> Optional[pd.DataFrame]:
    if not files: return None
    rows: List[pd.DataFrame] = []
    def build(content: bytes, fname: str) -> Optional[pd.DataFrame]:
        df = _read_rk_nonbca_positional(content)
        if df is None or df.empty: return None
        if remark_codes:
            df = df.loc[_remark_mask_startswith_codes(df["Remark"], remark_codes)]
        if df.empty: return None
        out = df.rename(columns={"Tanggal":"Date"}).copy()
        out["Amount"] = out["Amount"].apply(_format_rp_signed)
        out.insert(0, "File", fname)
        return out[["File","Date","Remark","Amount"]]
    for f in files:
        try:
            data = getattr(f, "getvalue", f.read)()
        except Exception:
            ...
            if str(fname).lower().endswith(".zip"):
                with zipfile.ZipFile(io.BytesIO(data)) as zf:
                    for info in zf.infolist():
                        ...
                        part = build(zf.read(info), info.filename)
                        if part is not None and not part.empty: rows.append(part)
            else:
                part = build(data, fname)
                if part is not None and not part.empty: rows.append(part)
        except Exception:
            continue
    if not rows: return None
    show = pd.concat(rows, ignore_index=True)
    # tampilkan tanpa kolom File agar sesuai screenshot
    return show[["Date","Remark","Amount"]].head(max_rows)

# (opsional) Preview ringkas/grouped (tetap dipakai di bawah bila perlu)
def _preview_rk_nonbca_no_account(files: List["st.runtime.uploaded_file_manager.UploadedFile"],
                                  remark_codes: List[str],
                                  max_rows: int = 50) -> Optional[pd.DataFrame]:
    if not files: return None
    previews: List[pd.DataFrame] = []
    def build_preview_df(content: bytes, fname: str) -> Optional[pd.DataFrame]:
        df = _read_rk_nonbca_positional(content)
        if df is None or df.empty: return None
        if remark_codes: df = df.loc[_remark_mask_startswith_codes(df["Remark"], remark_codes)]
        if df.empty: return None
        grp = (df.assign(Date=df["Tanggal"].dt.date)
                 .groupby("Date", as_index=False)
                 .agg(Amount=("Amount","sum"), Remark=("Remark","first")))
            ...
            if str(fname).lower().endswith(".zip"):
                with zipfile.ZipFile(io.BytesIO(data)) as zf:
                    for info in zf.infolist():
                        ...
                        if part is not None and not part.empty: previews.append(part)
            else:
...
def _read_rek_koran_base(content: bytes, remark_codes: List[str]) -> Optional[pd.DataFrame]:
    ...
    df_filt = df.loc[_remark_mask_contains_codes(df[remark_col].astype(str), remark_codes)] if remark_codes else df.copy()
    ...
    amt = pd.to_numeric(amt_raw.str.replace(r"[^\d\-]", "", regex=True), errors="coerce").fillna(0.0)
    df_filt["Amount"] = amt
    return df_filt
...
def _build_finnet_rekon_table(agg, df_finnet_settlement: Optional[pd.DataFrame], year: int, month: int,
    ...
    return out[final_cols].sort_values(["Pelabuhan","Tanggal"]).reset_index(drop=True)
...
def main() -> None:
    ...
    nonbca_codes = ["FINIF", "FINON"]  # prefix untuk Non BCA

    tabs_preview = st.tabs(["Non BCA", "BCA"])
    with tabs_preview[0]:
        if rek_nonbca_files:
            st.markdown("### 🔎 Baris (dengan format ribuan + tanda)")
            with st.spinner("Membaca baris RK Non BCA…"):
                prev_rows = _preview_rk_nonbca_rows_formatted(rek_nonbca_files, nonbca_codes, max_rows=200)
            if prev_rows is None or prev_rows.empty:
                st.info("Tidak ada baris yang terdeteksi untuk RK Non BCA (cek baris 13–1000, Date=A, Credit=J, atau remark diawali FINIF/FINON).")
            else:
                st.dataframe(prev_rows, use_container_width=True)

            st.markdown("### Rekap per Tanggal")
            with st.spinner("Membaca rekap tanggal RK Non BCA…"):

...
def _read_any_table_with_header(content: bytes, filename: str, header_row: int) -> Optional[pd.DataFrame]:
    # header bisa 13/14; engine berbeda per ekstensi
...
def _parse_amount_credit_series(s: pd.Series) -> pd.Series:
    # robust format: titik/koma, CR/DR, kurung, minus unicode
    x = s.astype(str)
    neg = (
...
# -------- Preview RK BCA (basis untuk loader BCA inflow) --------
...
def main() -> None:
    ...
    # Dana Masuk - BCA (dari RK BCA, FINIF)

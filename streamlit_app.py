...
XLSB_BATCH_ROWS = 100_000  # lebih besar, pyxlsb relatif murah memory
...
def _is_seekable(f) -> bool:
    try:
        f.seek(0, io.SEEK_CUR); return True
    except Exception:
        return False
...
def _reset_and_wrap_csv(f_or_bytes: Union[bytes, BinaryIO]) -> BinaryIO:
    if isinstance(f_or_bytes, (bytes, bytearray)):
        return io.BytesIO(f_or_bytes)
    try: f_or_bytes.seek(0)
    except Exception: return f_or_bytes
    return f_or_bytes

def _materialize_uploads(files: Optional[List["st.runtime.uploaded_file_manager.UploadedFile"]]) -> List[Tuple[str, bytes]]:
    payload: List[Tuple[str, bytes]] = []
    if not files: return payload
    for f in files:
        name = getattr(f, "name", ""); if not name: continue
        data: Optional[bytes] = None
        try: data = f.getvalue()
        except Exception:
            try:
                f.seek(0); raw = f.read(); data = raw.encode() if isinstance(raw, str) else raw
            except Exception:
                continue
...
def _read_bca_table_row2(content: Union[bytes, BinaryIO]) -> Optional[pd.DataFrame]:
    ...
    for eng in ("openpyxl", "xlrd", "pyxlsb", None):
        try:
            if eng: df = pd.read_excel(_ensure_seekable(content), engine=eng, header=0)
            else:   df = pd.read_excel(_ensure_seekable(content), header=0)
            break
        except Exception:
            df = None
    if df is None:
        try: df = pd.read_csv(_reset_and_wrap_csv(content), header=0)
        except Exception: return None
    return df if (df is not None and not df.empty) else None

def _to_date_minus1(v) -> pd.Series:
    t = pd.to_datetime(v, errors="coerce", dayfirst=True); return t - pd.Timedelta(days=1)
...
def _apply_rules_and_update_v2(df_chunk: pd.DataFrame, agg) -> None:
    ...
    # penting: regex=False untuk fixed-string (lebih cepat)
    m_cash   = H.str.contains("cash", na=False, regex=False)
    m_bri    = H.str.contains("prepaid-bri", na=False, regex=False)
    m_bni    = H.str.contains("prepaid-bni", na=False, regex=False)
    m_mand   = H.str.contains("prepaid-mandiri", na=False, regex=False)
    m_bca    = H.str.contains("prepaid-bca", na=False, regex=False)
    m_skpt   = H.str.contains("skpt", na=False, regex=False)
    m_ifcs   = H.str.contains("ifcs", na=False, regex=False)
    m_red    = H.str.contains("reedem", na=False, regex=False) | H.str.contains("redeem", na=False, regex=False)
    m_finpay = H.str.contains("finpay", na=False, regex=False)
    m_aa_esp = AA.str.startswith("esp", na=False)

    m_bca_tag  = X.str.contains("vabcaespay", na=False, regex=False) | X.str.contains("bluespay", na=False, regex=False)
    m_spay     = X.str.contains("spay", na=False, regex=False)
    m_not_spay = ~m_spay
    m_bca_x    = X.str.contains("bca", na=False, regex=False)
    m_non_bca_x = ~m_bca_x
...
# ---------- FAST datetime parser untuk Payment Date ----------
def _fast_excel_datetime(s: pd.Series) -> pd.Series:
    """
    Why: mempercepat parsing tanggal kolom payment dari .xlsb/.xlsx
    - Jika numeric seperti serial Excel (20_000..60_000), gunakan origin 1899-12-30.
    - Fallback ke to_datetime biasa (dayfirst=True).
    """
    if pd.api.types.is_numeric_dtype(s):
        return pd.to_datetime(s, unit="D", origin="1899-12-30", errors="coerce")
    s_num = pd.to_numeric(s, errors="coerce")
    if s_num.notna().sum() >= 0.7 * len(s):
        # mayoritas numeric → treat as Excel serial
        return pd.to_datetime(s_num, unit="D", origin="1899-12-30", errors="coerce")
    return pd.to_datetime(s, errors="coerce", dayfirst=True)

# ---------- CSV/XLSX/XLSB readers ----------
def _process_csv_fast(data_or_buf: Union[bytes, BinaryIO], year: int, month: int, agg) -> None:
    ...
    for chunk in itr:
        try:
            t = _fast_excel_datetime(chunk[COL_B])
...
def _flush_xlsx_batch(buf: List[List], year: int, month: int, agg) -> None:
    ...
    t = _fast_excel_datetime(df[COL_B])
    mask = (t.dt.year == year) & (t.dt.month == month)
...
def _process_xlsb(data_or_buf: Union[bytes, BinaryIO], year: int, month: int, agg) -> None:
    """Optimized pyxlsb streaming: direct index access, big batch, fast datetime."""
    try:
        try:
            from pyxlsb import open_workbook
        except ImportError:
            st.warning("Butuh pyxlsb untuk membaca .xlsb (`pip install pyxlsb`)."); return
        ...
        with open_workbook(fh) as wb:
            ...
            with wb.get_sheet(sheetname) as ws:
                ...
                if not all(c in name_to_idx for c in REQUIRED_COLS): return

                idx_h   = name_to_idx[COL_H]
                idx_b   = name_to_idx[COL_B]
                idx_aa  = name_to_idx[COL_AA]
                idx_k   = name_to_idx[COL_K]
                idx_x   = name_to_idx[COL_X]
                idx_asl = name_to_idx[COL_ASAL]

                buf: List[List] = []
                append = buf.append  # micro-opt

                for r in rows:
                    try:
                        # akses langsung per indeks; hindari fungsi per-cell
                        v_h   = r[idx_h].v   if idx_h   < len(r) and r[idx_h]   is not None else None
                        v_b   = r[idx_b].v   if idx_b   < len(r) and r[idx_b]   is not None else None
                        v_aa  = r[idx_aa].v  if idx_aa  < len(r) and r[idx_aa]  is not None else None
                        v_k   = r[idx_k].v   if idx_k   < len(r) and r[idx_k]   is not None else None
                        v_x   = r[idx_x].v   if idx_x   < len(r) and r[idx_x]   is not None else None
                        v_asl = r[idx_asl].v if idx_asl < len(r) and r[idx_asl] is not None else None
                        append([v_h, v_b, v_aa, v_k, v_x, v_asl])
                    except Exception:
                        continue
                    if len(buf) >= XLSB_BATCH_ROWS:
                        _flush_xlsx_batch(buf, year, month, agg); buf.clear()

                if buf:
                    _flush_xlsx_batch(buf, year, month, agg); buf.clear()
    except Exception:
        # Fallback: pandas engine pyxlsb (lebih lambat)
        try:
...
def _process_payment_file_to_partial(name: str, content: bytes, year: int, month: int):
    ...
    try:
        ...
        if not low_name: return local_agg

        if low_name.endswith(".zip"):
            with zipfile.ZipFile(io.BytesIO(content)) as zf:
                # proses sequential di dalam zip (lebih aman memori); luar tetap paralel
                for m in zf.infolist():
                    ...
                    with zf.open(m, "r") as member:
                        if low.endswith((".xlsx", ".xls")):
                            _process_xlsx_streaming(member.read(), year, month, local_agg)
                        elif low.endswith(".xlsb"):
                            _process_xlsb(member.read(), year, month, local_agg)
                        else:
                            _process_csv_fast(member, year, month, local_agg)
...
def _load_and_aggregate(files: List[Tuple[str, bytes]], year: int, month: int, parallel: bool = True):
    agg = _empty_agg(); errors = 0
    if not files: return agg
...
def _read_settlement_single_table(content: Union[bytes, BinaryIO], filename: str) -> Optional[pd.DataFrame]:
    low = str(filename).lower(); df = None
    try:
        if low.endswith(".csv"):
            df = pd.read_csv(_reset_and_wrap_csv(content), sep=",")
        elif low.endswith(".xlsx"):
            try: df = pd.read_excel(_ensure_seekable(content), engine="openpyxl")
            except ImportError:
                st.warning("Butuh openpyxl untuk membaca .xlsx. Jalankan: `pip install openpyxl`"); return None
        else:
            return None
    except Exception:
        return None
                ...
                with zipfile.ZipFile(io.BytesIO(content)) as zf:
                    for info in zf.infolist():
                        ...
                        with zf.open(info, "r") as member:
                            content2 = member.read() if low.endswith(".xlsx") else member
                            df_part = _read_settlement_single_table(content2, low)
...
def _read_finnet_single_csv(content: Union[bytes, BinaryIO]) -> Optional[pd.DataFrame]:
    try: df = pd.read_csv(_reset_and_wrap_csv(content), sep=",")
    except Exception: return None
            ...
            if low_name.endswith(".zip"):
                with zipfile.ZipFile(io.BytesIO(content)) as zf:
                    for info in zf.infolist():
                        ...
                        with zipfile.ZipFile(io.BytesIO(content)) as _:
                            pass
                        with zf.open(info, "r") as member:
...
def _build_finnet_settlement_table(df_finnet: pd.DataFrame, year: int, month: int) -> pd.DataFrame:
    ...
    if missing:
        st.warning("Settlement Finnet: kolom berikut tidak ditemukan di file: " + ", ".join(missing)); return pd.DataFrame()
...
def _map_dict_to_df(d: Dict[Tuple[date, str], float], col: str) -> pd.DataFrame:
    if not d: return pd.DataFrame(columns=["Tanggal", "Pelabuhan", col])
    tmp = pd.DataFrame([(k[0], _canonical_port_name(k[1]), float(v)) for k, v in d.items()], columns=["Tanggal", "Pelabuhan", col])
...
def _build_finnet_rekon_table(agg, df_finnet_settlement: Optional[pd.DataFrame], year: int, month: int,
    ...
    return _build_gateway_rekon_table(
        agg=agg, df_settlement=df_finnet_settlement, year=year, month=month,
...
def _build_espay_rekon_table(agg, df_espay_settlement: Optional[pd.DataFrame], year: int, month: int,
    ...
    return _build_gateway_rekon_table(
        agg=agg, df_settlement=df_espay_settlement, year=year, month=month,
...
def main() -> None:
    ...
    settlement_files = st.sidebar.file_uploader(
        "Upload Settlement ESPAY (.xlsx / .csv)", type=["xlsx", "csv"], accept_multiple_files=True,
    ...
    if result.empty:
        st.warning("Tidak ada data valid setelah filter periode & kolom wajib."); return

    try: result["Pelabuhan"] = result["Pelabuhan"].astype("category")
    ...
    nonbca_finif_by_dt_port = _load_rk_nonbca_inflow_by_dt_port_from_files(rek_nonbca_payload, header_row=13) if rek_nonbca_payload else {}
    df_rekon_finnet = _build_finnet_rekon_table(
        agg, df_finnet, year=year, month=month,
        bca_inflow_by_dt_port=bca_finif_by_dt_port, nonbca_inflow_by_dt_port=nonbca_finif_by_dt_port,
    )
    ...
    nonbca_sgw_by_dt_port = _load_rk_nonbca_inflow_by_dt_port_from_files_sgw(rek_nonbca_payload, header_row=13) if rek_nonbca_payload else {}
    df_rekon_espay = _build_espay_rekon_table(
        agg, df_espay_for_rekon, year=year, month=month,
        bca_inflow_by_dt_port_sgw=bca_sgw_by_dt_port, nonbca_inflow_by_dt_port_sgw=nonbca_sgw_by_dt_port,
    )

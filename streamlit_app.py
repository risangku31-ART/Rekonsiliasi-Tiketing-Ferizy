...
def _read_settlement_single_table(content: bytes, filename: str) -> Optional[pd.DataFrame]:
    ...
    if c_amt is not None and c_fee is not None:
        amt = _parse_money_series(df[c_amt])
        fee = _parse_money_series(df[c_fee])
        net_series = (amt - fee)
    else:
...
def main() -> None:
    ...
    if not st.session_state.run_started:
        st.info("Unggah berkas apa saja, lalu klik **▶️ Mulai Proses**. Tiap step akan jalan walau step lain kosong.")
        return
...
if __name__ == "__main__":
    try: main()
        ...
        st.error("Aplikasi error saat render awal."); st.exception(e)

...
# RK Non BCA: jendela baris & kolom kredit
NONBCA_START_ROW = 13
NONBCA_END_ROW = 1000
NONBCA_CREDIT_COL_INDEX = 9  # Kolom J (1-based 10)
...
# -------- Preview RK Non BCA: Date, Remark, Amount (Credit@J), grouped by Date (dengan filter prefix) --------
...
def _preview_rk_nonbca_no_account(
    ...
    def build_preview_df(content: bytes, fname: str) -> Optional[pd.DataFrame]:
        ...
        if df.empty:
            return None
        grp = (
            df.assign(Date=df["Tanggal"].dt.date)
    ...
    with tabs_preview[0]:
        if rek_nonbca_files:
            ...
            if prev_nonbca_raw is None or prev_nonbca_raw.empty:
                st.info("Parser tidak menemukan data pada baris 13–1000 (Date=A, Credit=J).")
            else:
